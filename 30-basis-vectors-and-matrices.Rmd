...
In the chapter [Vectors](),  we learned that a vector could be represented in a coordinate system where each element in the vector corresponds to a distance along one of the reference axes defining the coordinate system. For example, the vector:
...
has a distance of $-3$ on the first reference axis (R1) and a distance of 4 on the second reference axis (R2). This vector and the reference axes are shown in Figure \@ref(fig:fig14-1). The reference axes (R1 and R2) define a two-dimensional coordinate space.

```{r fig14-1, echo=FALSE, out.width='60%', fig.cap="Plot showing vector **a** (in teal) in the R1--R2 dimensional space."}
knitr::include_graphics("figs/fig-14-01.png")
```

The two-dimensional coordinate system in our example, the Cartesian coordinate system, is defined by two orthogonal vectors called the *basis vectors* because they form the basis for the two-dimensional space. (These are shown in black in Figure \@ref(fig:fig14-1).) More specifically, the dimensions of a system are determined by the number of independent vectors within that system. For example, in a three-dimensional coordinate space there are three independent vectors (e.g., that form the *x*, *y*, and *z* axes).
...
What are the coordinates of this vector in the standard basis system? \@ref(fig:fig14-3) shows this graphically. 

```{r fig14-4, echo=FALSE, out.width='60%', fig.cap="Plot showing vector **a** (in teal) in the R1--R2 non-standard basis defined by **b1** and **b2**. The coordinates in the standard basis are found by determining the projection onto the span of the standard basis vectors (i.e., the R1 and R2 axes in the standard basis)."}
